...
class ts2pythonCompiler(Compiler):
    ...
    def prepare(self, root: Node) -> None:
        type_aliases = {
            nd["identifier"].content for nd in root.select_children("type_alias")
        }
        namespaces = {
            nd["identifier"].content for nd in root.select_children("namespace")
        }
    ...
    def on_map_signature(self, node) -> str:
        return "Dict[%s, %s]" % (
            self.compile(node["index_signature"]),
    ...
    def on_item(self, node) -> str:
        if len(node.children) == 1:
            ...
            return (
                self.compile(node["identifier"]) + " = " + self.compile(node["literal"])
            )
    ...
    def on_const(self, node) -> str:
        if "literal" in node or "identifier" in node:
            if self.strip_type_from_const:
                ...
                return (
                    self.compile(node["declaration"]) + " = " + self.compile(node[-1])
                )
        else:
...
def compile_src(source: str) -> Tuple[Any, List[Error]]:
    ...
    result_tuple = compile_source(
        source, get_preprocessor(), get_grammar(), get_transformer(), get_compiler()
    )
...
def serialize_result(result: Any) -> Union[str, bytes]:
    ...
    if isinstance(result, Node):
        return result.serialize(
            how="default" if RESULT_FILE_EXTENSION != ".xml" else "xml"
        )
...
def process_file(source: str, result_filename: str = "") -> str:
    ...
    if not has_errors(errors, FATAL):
        if os.path.abspath(source_filename) != os.path.abspath(result_filename):
            ...
            errors.append(
                Error(
                    'Source and destination have the same name "%s"!' % result_filename,
                    0,
                    FATAL,
                )
            )
...
def batch_process(
    ...
    *,
    ...
    def gen_dest_name(name):
        return os.path.join(
            out_dir, os.path.splitext(os.path.basename(name))[0] + RESULT_FILE_EXTENSION
        )
    ...
    if submit_func is None:
        ...
        with instantiate_executor(
            get_config_value("batch_processing_parallelization"),
...
def main():
    ...
    workdir = (
        ...
        else os.path.dirname(file_names[0])
    )
